...
object Log {
  ...
  def errln(msg: String, prependTimestamp: Boolean = true): Unit = { System.err.println(if (prependTimestamp) this.prependTimestamp(msg) else msg) }

  def println(msg: String, prependTimestamp: Boolean = true): Unit = { System.out.println(if (prependTimestamp) this.prependTimestamp(msg) else msg) }
  ...
  def error(msg: String*)(implicit context: LogContext): Unit = log(
    msg.mkString(MsgSeparator),
    LogLevels.Error,
    { msg =>
