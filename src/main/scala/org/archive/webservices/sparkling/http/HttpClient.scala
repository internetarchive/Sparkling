...
object HttpClient {
  ...
  def requestMessage[R](
      ...
      timeoutMillis: Int = DefaultTimeoutMillis,
      close: Boolean = true
  )(action: HttpMessage => R): R = rangeRequestMessage(url, headers, retries = retries, sleepMillis = sleepMillis, timeoutMillis = timeoutMillis, close = close)(action)
  ...
  )(action: URLConnection => R): R = rangeRequestConnection(url, headers, retries = retries, sleepMillis = sleepMillis, timeoutMillis = timeoutMillis, disconnect = disconnect)(action)
  ...
  def rangeRequestConnection[R](
    ...
    val connection = Common.retryObj(new URL(url).openConnection.asInstanceOf[HttpURLConnection])(
      ...
      _.disconnect,
      (_, retry, e) => {
        "Request failed (" + retry + "/" + retries + "): " + url + " (" + offset + "-" +
          (if (length >= 0) length else "") + ") - " + e.getClass.getCanonicalName + Option(e.getMessage).map(_.trim).filter(_.nonEmpty).map(" - " + _).getOrElse("")
      }
    ) { (connection, _) =>
