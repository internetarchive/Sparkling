...
object Sparkling {
  ...
  def resetSparkContext(sc: Option[SparkContext] = None): Unit = _sc = sc

  def scOpt: Option[SparkContext] = _sc.filter(!_.isStopped)

  def registerClasses(c: Class[_]*): Unit = for (sc <- scOpt) sc.getConf.registerKryoClasses(c.toArray)
