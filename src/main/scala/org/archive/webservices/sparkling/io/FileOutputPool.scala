...
import org.archive.webservices.sparkling.util.CacheMap
...
class FileOutputPool(minOpen: Int, maxOpen: Int) {
  ...
  def get(file: String): OutputStream = { cache.getOrElse(file, new AppendingFileOutputStream(new File(file), onOpen = stream => cache.cache(file, stream), onClose = _ => cache.remove(file))) }
