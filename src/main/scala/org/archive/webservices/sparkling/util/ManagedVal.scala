...
//      SparkUtil.removeTaskCleanup(this)
  ...
  def iter[B](iter: A => Iterator[B], cleanup: () => Unit = () => {}, lazyEval: Boolean = true, throwOnError: Boolean = false): ManagedVal[CleanupIterator[B]] = ManagedVal(
    {
      either match {
        case Right(v) => CleanupIterator(
            iter(v),
            () => {
              cleanup()
              clear()
