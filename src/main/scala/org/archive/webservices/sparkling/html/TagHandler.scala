...
private class SpecificTagHandler[R](val tags: Set[String], hierarchical: Boolean, handleClosing: Boolean, initial: R, handler: (TagMatch, Seq[TagMatch], R) => R)
    extends AllTagHandler[R](hierarchical, handleClosing, initial, handler) {
  override def handles(tag: String): Boolean = tags.contains(tag)
}

object TagHandler {
  def noop(tags: Set[String]): TagHandler[_] = new NoopTagHandler(tags.map(_.toLowerCase))
  def all[R](initial: R, hierarchical: Boolean = false, handleClosing: Boolean = false)(handler: (TagMatch, Seq[TagMatch], R) => R): TagHandler[R] =
    new AllTagHandler(hierarchical, handleClosing, initial, handler)
  def apply[R](tags: Set[String], initial: R, hierarchical: Boolean = true, handleClosing: Boolean = false)(handler: (TagMatch, Seq[TagMatch], R) => R): TagHandler[R] =
    new SpecificTagHandler(tags.map(_.toLowerCase), hierarchical, handleClosing, initial, handler)
}
